  ...
  <div class="container-fluid" v-if="productsInCart.length !== 0">
    <div class="row">
      ...
      <div class="button_space col-10 mx-auto d-flex justify-content-md-end justify-content-sm-center">
        <button class="check_button col-md-3 h4 fw-bold text-center py-3 px-5 col-sm-12" @click="check(productsInCart)">
          確認結帳
        </button>
      </div>
    </div>
    ...
    methods: {
      ...
      check (product) {
        if (this.productsInCart.length == 0) {
          ...
          product.orderId = new Date().getTime();
          this.$router.push (`/bag/${product.orderId}`);
          return;
        }
      },
    },
